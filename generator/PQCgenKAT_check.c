...
int main(void) {
	...
	do {
		...
		/* m may legitimately be NULL from calloc(0, ...) when mlen == 0 (empty
		 * message); m1/sm/sm2 are always calloc'd with a strictly positive size
		 * (mlen + CRYPTO_BYTES), so NULL there is a genuine allocation failure. */
		if ( ((m == NULL) && (mlen != 0)) || (m1 == NULL) || (sm == NULL) || (sm2 == NULL) ) {
			printf("ERROR: memory allocation failure for mlen = %llu\n", mlen);
			return KAT_DATA_ERROR;
		}

		if ( !ReadHex(fp_rsp, m, (size_t)mlen, "msg = ") ) {
		...
		if ( smlen > mlen + CRYPTO_BYTES ) {
			printf("ERROR: 'smlen' (%llu) exceeds the allocated 'sm' buffer (mlen + CRYPTO_BYTES = %llu) in <%s>\n", smlen, mlen + (unsigned long long)CRYPTO_BYTES, fn_rsp);
			return KAT_DATA_ERROR;
		}

		if ( !ReadHex(fp_rsp, sm, (size_t)smlen, "sm = ") ) {
...
//
int
ReadHex(FILE *infile, unsigned char *A, size_t Length, char *str) {
	size_t      i;
	int         ch, started;
	unsigned char   ich;
