...
int main(void) {
	...
	/* +1: the read loop below probes one index past the expected signature
	 * length to detect EOF; that probe must land inside the buffer. */
	uint8_t sm[MLEN + CRYPTO_BYTES + 1];
	...
	smlen = 0;
	int val = 1;
	while ((val > 0) && (smlen <= (unsigned long long)(MLEN + CRYPTO_BYTES))) {
		val = fscanf(fptr, "%hhu ", &sm[smlen]);
		smlen++;
	}
	smlen--;
	fclose(fptr);
	if (smlen != (unsigned long long)(MLEN + CRYPTO_BYTES)) {
		printf("Failure: unexpected signature length in 'bench-sig.txt' (got %llu, expected %lld)\n", smlen, (long long)(MLEN + CRYPTO_BYTES));
		return 0;
	}
