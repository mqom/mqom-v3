...
BASE_FIELDS = {'gf2': '1', 'gf16': '4'}
TRADE_OFFS = {'fast': '0', 'short': '1', 'shorter': '2'}
VARIANTS = {'ct': '0', 'ot': '1'}

# Not all (field, tradeoff) pairs are valid: gf2 only supports 'shorter',
# gf16 only supports 'fast' and 'short'.
VALID_FIELD_TRADEOFFS = {
    'gf2':  {'shorter'},
    'gf16': {'fast', 'short'},
}

# List all choices (only valid field/tradeoff combinations)
...
for cat in CATEGORIES:
    ...
    for field in BASE_FIELDS:
        ...
        for tradeoff in TRADE_OFFS:
            if tradeoff not in VALID_FIELD_TRADEOFFS[field]:
                continue
...
parser_compile.set_defaults(b_no_openssl=False)
parser_compile.add_argument('--static', action='store_true', dest='b_static', help='Pass -static to the linker (useful for cross-compilation targeting bare-metal or QEMU without sysroot)')
...
parser_compile.add_argument('--tmpdir', dest='tmpdir', default=None, help='Use this directory as temporary build root instead of /tmp (or $TMPDIR); useful when /tmp is a tmpfs under memory pressure')
...
# Commands run with check=True that exited non-zero. Appending to a list is
# atomic enough for the joblib "threading" backend used by the parallel paths.
COMMAND_FAILURES = []

# Utility to execute command
def run_command(command, cwd, shell=False, check=False):
    import subprocess
    if shell:
    ...
    # Without check=True the exit status is ignored, which is what the bench and
    # valgrind paths want (they are read through their output). Build and KAT
    # steps pass check=True: a failed link used to go unnoticed here and was only
    # caught later, indirectly, by the missing binary.
    if check and process.returncode != 0:
        COMMAND_FAILURES.append((command, process.returncode))
        marker = f'[manage.py] command exited with code {process.returncode}: {command}\n'
        stderr = marker + (stderr or '')
    return stdout, stderr

# Make the process exit status reflect the checked commands that failed, so a
# caller (CI, shell loop) sees the failure instead of a silent success.
def report_command_failures(what):
    if not COMMAND_FAILURES:
        return
    print(f'\n[manage.py] {len(COMMAND_FAILURES)} {what} step(s) failed:')
    for cmd, code in COMMAND_FAILURES:
...
class MQOMInstance:
    def __init__(self, scheme, dst_path):
        ...
        new_lst = [item for item in lst if '-DMQOM3_PARAM_' not in item]
        new_lst.append('-DMQOM3_PARAM_SECURITY='+CATEGORIES[cat])
        new_lst.append('-DMQOM3_PARAM_BASE_FIELD='+BASE_FIELDS[field])
        new_lst.append('-DMQOM3_PARAM_TRADEOFF='+TRADE_OFFS[tradeoff])
        new_lst.append('-DMQOM3_PARAM_OT_VARIANT='+VARIANTS[variant])
        if getattr(arguments, 'b_static', False):
            new_lst.append('-static')
    ...
    def compile_bench(self, folder):
        ...
        if arguments.b_compile_only_print:
            ...
            print(f'MQOM3_VARIANT="{prefix_exec}" EXTRA_CFLAGS="{extra_cflags}" DESTINATION_PATH="{dst_path}" PREFIX_EXEC="{prefix_exec}" make bench')
            return "",""
        else:
            return run_command(f'MQOM3_VARIANT="{prefix_exec}" EXTRA_CFLAGS="{extra_cflags}" DESTINATION_PATH="{dst_path}" PREFIX_EXEC="{prefix_exec}" make bench', folder, shell=True, check=True)
    ...
    def compile_bench_mem_keygen(self, folder):
        ...
        if arguments.b_compile_only_print:
            ...
            print(f'MQOM3_VARIANT="{prefix_exec}" EXTRA_CFLAGS="{extra_cflags}" DESTINATION_PATH="{dst_path}" PREFIX_EXEC="{prefix_exec}" make bench_mem_keygen')
            return "",""
        else:
            return run_command(f'MQOM3_VARIANT="{prefix_exec}" EXTRA_CFLAGS="{extra_cflags}" DESTINATION_PATH="{dst_path}" PREFIX_EXEC="{prefix_exec}" make bench_mem_keygen', folder, shell=True, check=True)
    ...
    def compile_bench_mem_sign(self, folder):
        ...
        if arguments.b_compile_only_print:
            print("=== Compilation (bench write) of %s" % prefix_exec)
            print(f'MQOM3_VARIANT="{prefix_exec}" EXTRA_CFLAGS="{extra_cflags}" DESTINATION_PATH="{dst_path}" PREFIX_EXEC="{prefix_exec}" make bench_mem_sign')
            return "",""
        else:
            return run_command(f'MQOM3_VARIANT="{prefix_exec}" EXTRA_CFLAGS="{extra_cflags}" DESTINATION_PATH="{dst_path}" PREFIX_EXEC="{prefix_exec}" make bench_mem_sign', folder, shell=True, check=True)
    ...
    def compile_bench_mem_open(self, folder):
        ...
        if arguments.b_compile_only_print:
            ...
            print(f'MQOM3_VARIANT="{prefix_exec}" EXTRA_CFLAGS="{extra_cflags}" DESTINATION_PATH="{dst_path}" PREFIX_EXEC="{prefix_exec}" make bench_mem_open')
            return "",""
        else:
            return run_command(f'MQOM3_VARIANT="{prefix_exec}" EXTRA_CFLAGS="{extra_cflags}" DESTINATION_PATH="{dst_path}" PREFIX_EXEC="{prefix_exec}" make bench_mem_open', folder, shell=True, check=True)
    ...
    def compile_kat_gen(self, folder):
        ...
        if arguments.b_compile_only_print:
            ...
            print(f'MQOM3_VARIANT="{prefix_exec}" EXTRA_CFLAGS="{extra_cflags}" DESTINATION_PATH="{dst_path}" PREFIX_EXEC="{prefix_exec}" make kat_gen')
            return "",""
        else:
            return run_command(f'MQOM3_VARIANT="{prefix_exec}" EXTRA_CFLAGS="{extra_cflags}" DESTINATION_PATH="{dst_path}" PREFIX_EXEC="{prefix_exec}" make kat_gen', folder, shell=True, check=True)

    def compile_embedded_kat(self, folder):
        extra_cflags = self.compilation_prefix['EXTRA_CFLAGS']
        prefix_exec = self.get_label()
        dst_path = self.dst_path
        if arguments.b_compile_only_print:
            print("=== Compilation (embedded KAT) of %s" % prefix_exec)
            print(f'MQOM3_VARIANT="{prefix_exec}" EXTRA_CFLAGS="{extra_cflags}" DESTINATION_PATH="{dst_path}" PREFIX_EXEC="{prefix_exec}" make test_embedded_KAT')
            return "",""
        else:
            return run_command(f'MQOM3_VARIANT="{prefix_exec}" EXTRA_CFLAGS="{extra_cflags}" DESTINATION_PATH="{dst_path}" PREFIX_EXEC="{prefix_exec}" make test_embedded_KAT', folder, shell=True, check=True)
    ...
    def compile_kat_check(self, folder):
        ...
        if arguments.b_compile_only_print:
            ...
            print(f'MQOM3_VARIANT="{prefix_exec}" EXTRA_CFLAGS="{extra_cflags}" DESTINATION_PATH="{dst_path}" PREFIX_EXEC="{prefix_exec}" make kat_check')
            return "",""
        else:
            return run_command(f'MQOM3_VARIANT="{prefix_exec}" EXTRA_CFLAGS="{extra_cflags}" DESTINATION_PATH="{dst_path}" PREFIX_EXEC="{prefix_exec}" make kat_check', folder, shell=True, check=True)

    def run_bench(self, nb_experiments):
        scheme_label = self.get_label()
        dst_path = self.dst_path
        bench_bin = f'{dst_path}/{scheme_label}_bench'
        if not os.path.exists(bench_bin):
            print(f'[-] Error: bench binary not found: {bench_bin}', file=sys.stderr)
            print(f'    Compile first: python3 manage.py compile {scheme_label}', file=sys.stderr)
            sys.exit(1)
        ...
        try:
            ...
            pass
        try:
            kg_stack = int(get_info(stdout, r' - Key Gen stack: (\d+) B'))
            sg_stack = int(get_info(stdout, r' - Sign stack: (\d+) B'))
            vf_stack = int(get_info(stdout, r' - Verify stack: (\d+) B'))
            kg_heap  = int(get_info(stdout, r' - Key Gen heap: (\d+) B'))
            sg_heap  = int(get_info(stdout, r' - Sign heap: (\d+) B'))
            vf_heap  = int(get_info(stdout, r' - Verify heap: (\d+) B'))
            data['memory'] = {
                'keygen': {'stack': kg_stack, 'heap': kg_heap},
                'sign':   {'stack': sg_stack, 'heap': sg_heap},
                'verify': {'stack': vf_stack, 'heap': vf_heap},
            }
        except ValueError:
            pass
        if arguments.b_verbose:
    ...
    def run_kat_gen(self):
        scheme_label = self.get_label()
        dst_path = self.dst_path
        return run_command(f'cd {dst_path} && ./{scheme_label}_kat_gen', cwd=CWD, shell=True, check=True)

    def run_kat_check(self):
        scheme_label = self.get_label()
        dst_path = self.dst_path
        return run_command(f'cd {dst_path} && ./{scheme_label}_kat_check', cwd=CWD, shell=True, check=True)

    def run_valgrind_bench(self):
        scheme_label = self.get_label()
        dst_path = self.dst_path
        command = f'valgrind --max-stackframe=10000000 --leak-check=yes {dst_path}/{scheme_label}_bench 1'
        _, stderr = run_command(command, cwd=CWD)
        # No summary line at all means valgrind never ran to completion (not installed,
        # binary missing, killed): report it instead of raising IndexError on [0].
        summaries = [line for line in (stderr or '').split('\n') if 'ERROR SUMMARY' in line]
        if not summaries:
            COMMAND_FAILURES.append((f'{command}  [no ERROR SUMMARY in output]', 1))
            return 'no ERROR SUMMARY (valgrind did not run to completion)'
        summary = summaries[0]
        # Printing the count was not enough: a caller reading only the exit status saw a
        # clean run whatever valgrind found. Register non-zero counts as failures so that
        # report_command_failures() below makes them reach the exit code, like the build
        # and KAT steps already do.
        found = re.search(r'ERROR SUMMARY:\s*(\d+)\s+error', summary)
        if (found is None) or (int(found.group(1)) != 0):
            COMMAND_FAILURES.append((f'{command}  [{summary.strip()}]', 1))
        return summary
...
if arguments.command == 'compile':
    ...
    def handle_scheme_compile(scheme):
        ...
        scheme.clean()
        base_tmp = arguments.tmpdir if arguments.tmpdir is not None else os.environ.get('TMPDIR', '/tmp')
        tname = base_tmp + '/' + scheme.get_label()
        ...
        try:
            ...
            if not arguments.b_no_kat:
                if arguments.b_no_openssl:
                    stdout, stderr = scheme.compile_embedded_kat(tname)
                    if arguments.b_verbose or stderr:
                        print(stdout, stderr)
                else:
            ...
            # Clean up immediately after this scheme is done so /tmp never
            # accumulates more than one (sequential) or N (parallel) copies.
            shutil.rmtree(tname, ignore_errors=True)
            try:
                tempdir_names.remove(tname)
            except ValueError:
                pass
    # Register the signal handler
    signal.signal(signal.SIGINT, signal_handler)
    ...
    report_command_failures('compilation')
    ...
    def handle_scheme_test(scheme):
        ...
        # Surface the clean per-command report before the assert below turns an
        # exit-status failure into a bare AssertionError.
        report_command_failures('test')
        assert (not stderr), stderr
        if arguments.b_verbose:
            print(stdout)
        ...
        if KAT_Folder is not None:
            ...
            if compare_kat_zip:
                scheme_name = 'mqom3_%s' % scheme.get_label()
            else:
